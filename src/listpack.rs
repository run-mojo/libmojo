...
impl Listpack {
    ...
    pub fn insert<V: Into<Value>>(
        ...
        match insert(self.0, v.into(), place, target) {
            Some((lp, ele)) => {
                self.0 = lp;
                Some(ele)
            }
...
//    }
    ...
    pub fn insert_signed_int<T>(
        ...
        where
            T: Int {
        match insert_signed_int(self.0, v, place, target) {
            Some((lp, ele)) => {
                self.0 = lp;
                Some(ele)
            }
    ...
    pub fn insert_string<T>(
        ...
        where
            T: Str {
        match insert_string(self.0, v, place, target) {
            Some((lp, ele)) => {
                self.0 = lp;
                Some(ele)
            }
    ...
    pub fn replace_signed_int<T>(
        ...
        where
            T: Int {
        match replace_signed_int(self.0, p, v) {
            Some((lp, ele)) => {
                self.0 = lp;
                Some(ele)
            }
    ...
    pub fn replace_string<T>(
        ...
        where
            T: Str {
        match replace_string(self.0, p, v) {
            Some((lp, ele)) => {
                self.0 = lp;
                Some(ele)
            }
    ...
    pub fn append_val2<T: AsRef<Value>>(
        ...
        match append_val(self.0, v.as_ref()) {
            (vv, Some(lp)) => {
                self.0 = lp;
                true
            }
    ...
    pub fn append_signed_int<T>(
        ...
        where
            T: Int {
        match append_signed_int(self.0, v) {
            Some(lp) => {
                self.0 = lp;
                true
            }
...
fn layout_for_size(size: usize) -> Layout {
    unsafe { Layout::from_size_align_unchecked(size, mem::size_of::<usize>()) }
}

fn alloc(size: usize) -> *mut u8 {
    unsafe { super::ALLOC.alloc(layout_for_size(size)) }
}

fn realloc(p: *mut u8, old_size: usize, new_size: usize) -> *mut u8 {
    unsafe {
        super::ALLOC.realloc(p, layout_for_size(old_size), new_size)
    }
}

fn dealloc(lp: *mut u8) {
    if !lp.is_null() {
        let size = get_total_bytes(lp) as usize;
        if size > 0 {
            unsafe {
                super::ALLOC.dealloc(lp, layout_for_size(size))
            }
        }
    }
}
...
pub fn insert(
    ...
    unsafe {
        ...
        lp = realloc(lp, old_listpack_bytes as usize, new_listpack_bytes as usize);
        if lp.is_null() {
            return None;
        }
...
pub fn insert_int<I>(
    ...
    where
        I: Int {
    insert(lp, Value::Int(v.to_int64()), place, target)
}

///
#[inline]
pub fn insert_signed_int<I>(
    lp: listpack,
    v: I,
    place: Placement,
    target: element,
    ...
    where
        I: Int {
    insert(lp, Value::Int(zigzag(v.to_int64())), place, target)
}
...
pub fn insert_string<S>(
    ...
    where
        S: Str {
    insert(lp, v.as_value(), place, target)
}
        ...
        if delta > 0 {
            ...
            match next(lp, p) {
                Some(ele) => {
                    ...
                    lp = realloc(lp, old_listpack_bytes as usize, new_listpack_bytes);
                    if lp.is_null() {
                        return None;
                    }
...
pub fn replace_int<I>(
    ...
    where
        I: Int {
    replace(lp, p, Value::Int(v.to_int64()))
}

///
#[inline]
pub fn replace_signed_int<I>(
    lp: listpack,
    p: element,
    v: I,
) -> Option<(listpack, element)>
    where
        I: Int {
    replace(lp, p, Value::Int(zigzag(v.to_int64())))
}

///
#[inline]
pub fn replace_string<S>(
    lp: listpack,
    p: element,
    mut v: S,
) -> Option<(listpack, element)>
...
pub fn revert(lp: listpack, size: u32, count: u16) {
...
pub fn append_writes<A>(
    ...
    unsafe {
        ...
        if new_listpack_bytes > layout_size {
            ...
            // realloc to make room
            lp = realloc(lp, layout_size as usize, new_listpack_bytes as usize);
            if lp.is_null() {
                return WriteResult(None, ptr::null_mut(), 0);
            }
        }
...
pub fn append_int<I>(
    ...
    where I: Int {
    append(lp, Value::Int(v.to_int64()))
}
...
#[inline(always)]
pub fn append_signed_int<I>(
    lp: listpack,
    v: I,
) -> Option<listpack>
    where I: Int {
    append(lp, Value::Int(zigzag(v.to_int64())))
}

///
#[inline(always)]
pub fn append_string<S>(
    lp: listpack,
    mut v: S,
) -> Option<listpack>
    where S: Str {
    append(lp, v.as_value())
}
...
//impl Allocator for super::jemallocator::Jemalloc {
//    fn alloc(&self, size: usize) -> *mut u8 {
//        unsafe {
//            self.alloc(Layout::from_size_align_unchecked(size, mem::size_of::<usize>()))
//        }
//    }
//
//    fn realloc(&self, ptr: *mut u8, size: usize) -> *mut u8 {
//        unsafe {
//            super::ALLOC.realloc(Layout::from_size_align_unchecked(size, mem::size_of::<usize>()))
//        }
//    }
//
//    fn dealloc(&self, lp: *mut u8) {
...
mod tests {
    ...
    fn test_it() {
        unsafe {
            ...
            lp = append(lp, Value::Int(20)).expect("");
            lp = append(lp, Value::Int(21)).expect("");
            lp = append(lp, Value::Int(22)).expect("");

//            lp = append(ALLOCATOR, lp, 20f64.into()).expect("");

            let s = "hello";
            lp = append(lp, Value::String(s.as_ptr(), s.len() as u32)).expect("");

            let mut p = first(lp as *mut u8).expect("No First!");

            let (mut lp, mut p) = insert(lp, Value::Int(1), Placement::After, p).unwrap();
            let (mut lp, mut p) = insert(lp, Value::Int(2), Placement::After, p).unwrap();
            let (mut lp, mut p) = insert(lp, Value::Int(3), Placement::After, last(lp).unwrap()).unwrap();
