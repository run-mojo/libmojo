macro_rules! non_null {
    ( $obj:expr, $ctx:expr ) => {
        if $obj.is_null() {
            return Err($crate::jni::wrapper::errors::ErrorKind::NullPtr($ctx).into());
        } else {
            $obj
        }
    }
...
macro_rules! deref {
    ( $obj:expr, $ctx:expr ) => {
        if $obj.is_null() {
            return Err($crate::jni::wrapper::errors::ErrorKind::NullDeref($ctx).into());
        } else {
            *$obj
        }
    };
    ...
    ( $jnienv:expr, $name:tt ) => ({
        ...
        match deref!(deref!(env, "JNIEnv"), "*JNIEnv").$name {
            ...
            None => {
                ...
                return Err($crate::jni::wrapper::errors::Error::from(
                    $crate::jni::wrapper::errors::ErrorKind::JNIEnvMethodNotFound(
                        stringify!($name))).into())},
        }
    })
}
...
macro_rules! check_exception {
    ( $jnienv:expr ) => {
        ...
        } == $crate::jni::sys::JNI_TRUE;
        if check {
            trace!("exception found, returning error");
            return Err($crate::jni::wrapper::errors::Error::from(
                $crate::jni::wrapper::errors::ErrorKind::JavaException).into());
        }
    ...
    ( $jnienv:expr, $name:tt ) => ({
        ...
        match deref!(deref!(env, "JavaVM"), "*JavaVM").$name {
            ...
            None => {
                ...
                return Err($crate::jni::wrapper::errors::Error::from(
                    $crate::jni::wrapper::errors::ErrorKind::JavaVMMethodNotFound(
                        stringify!($name))).into())},
        }
    })
}
